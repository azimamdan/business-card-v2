...
export function DashboardMobileTabs({ profile, blocks, editorContent }: DashboardMobileTabsProps) {
    ...
    return (
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            <div className="mb-6 sticky top-16 z-40 bg-background py-2">
                <TabsList className="grid w-full grid-cols-2 bg-card border border-border">
                    <TabsTrigger value="edit" className="data-[state=active]:bg-muted data-[state=active]:text-foreground text-muted-foreground">Edit Profile</TabsTrigger>
                    <TabsTrigger value="preview" className="data-[state=active]:bg-muted data-[state=active]:text-foreground text-muted-foreground">Live Preview</TabsTrigger>
                </TabsList>
            </div>
